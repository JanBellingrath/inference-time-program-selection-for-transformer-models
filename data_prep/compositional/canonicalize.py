...
from core.edit_dsl_compat import legacy_key_to_program  # noqa: E402
from data_prep.common.io import iter_jsonl, load_json, save_json  # noqa: E402
...
def _route_from_dev_entry(anchor: Sequence[int], dev: Dict[str, Any]) -> Optional[List[int]]:
    ...
    try:
        prog = legacy_key_to_program(str(key))
...
def canonicalize_directory(
    ...
    *,
    ...
    save_json(output_dir / "config.json", out_cfg, indent=2)
    ...
    for jsonl_path in jsonl_files:
        ...
        for suffix in ("pivot_residuals", "full_residuals"):
            residual_src = data_dir / f"{bench}_{suffix}.pt"
            if residual_src.exists():
                _link_residuals(
                    residual_src, output_dir / f"{bench}_{suffix}.pt", copy=copy_residuals
                )
    ...
    write_manifest(output_dir / "canonicalization_summary.json", summary)
    return summary
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--copy-residuals", action="store_true",
                   help="Copy residual .pt files (pivot and full) instead of symlinking.")
    p.add_argument(
        "--include_assign",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Include MCTS-style assign(pos,value) primitives (default: on; use --no-include_assign to disable).",
    )
    p.add_argument(
