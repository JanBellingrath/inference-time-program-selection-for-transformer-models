...
from data_prep.common.io import iter_jsonl, load_json  # noqa: E402
...
def _write_primitives_jsonl(path: Path, survivors: Sequence[Tuple[Primitive, Dict[str, Any]]]) -> None:
    with open(path, "w") as f:
        for idx, (prim, row) in enumerate(survivors):
            ...
            if "raw_count" in row:
                support["raw_count"] = int(row.get("raw_count", 0))
            entry = {
                "idx": idx,
                "kind": prim.kind,
                "args": list(prim.args),
...
def _load_geometry(data_dir: Path) -> Dict[str, Any]:
    ...
    include_assign = bool(canon.get("include_assign", True))
...
def build_catalogues(
    ...
    *,
    ...
    validate_no_duplicate_keys(primitive_key_to_idx.keys(), name="primitive_key")
