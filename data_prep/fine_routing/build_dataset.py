...
For each benchmark *b*:
  1.  Choose anchor sequence ``s_b``: default ``[0..L-1]``, or from MCTS
      snapshots (``--mcts_anchor_source benchmark_mcts``).
...
def resolve_anchor_sequences_for_build(
    cfg: FineRoutingConfig,
    *,
    wrapper: FlexibleModelWrapper,
    benchmarks: List[str],
) -> Dict[str, List[int]]:
    """Return ``benchmark -> anchor_seq`` for dataset construction."""
    if cfg.mcts_anchor_source not in ("default", "benchmark_mcts"):
        raise ValueError(f"unknown mcts_anchor_source: {cfg.mcts_anchor_source!r}")
    if cfg.mcts_anchor_source == "default":
        n = int(wrapper.num_layers)
        seq = list(range(n))
        return {b: list(seq) for b in benchmarks}
    anchor_seqs = load_optimal_sequences_from_results(
        cfg.results_dir, benchmarks, model_name=cfg.model_name,
    )
    missing = [b for b in benchmarks if b not in anchor_seqs]
    if missing:
        raise SystemExit(
            f"--mcts_anchor_source benchmark_mcts: no snapshot in {cfg.results_dir!r} "
            f"for benchmarks {missing} (model_name={cfg.model_name!r})"
        )
    return {b: [int(x) for x in anchor_seqs[b]] for b in benchmarks}
...
def parse_args() -> FineRoutingConfig:
    ...
    p.add_argument(
        "--mcts_anchor_source",
        choices=["default", "benchmark_mcts"],
        default="default",
        help=(
            "Root sequence for fine-grained MCTS: "
            "'default' = [0..L-1] (same for every benchmark); "
            "'benchmark_mcts' = per-benchmark optimal sequence from --results_dir "
            "(legacy)."
        ),
    )
    ...
    cfg.mcts_anchor_source = str(args.mcts_anchor_source)
...
def main():
    ...
    anchor_seqs = resolve_anchor_sequences_for_build(
        cfg, wrapper=wrapper, benchmarks=list(cfg.benchmarks),
    )
    logger.info(
        "MCTS anchor source=%s; anchors for: %s",
        cfg.mcts_anchor_source,
        list(anchor_seqs.keys()),
    )
