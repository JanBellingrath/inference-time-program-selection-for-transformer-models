#!/usr/bin/env python
"""
Benchmark-level MCTS sweep for Qwen 2.5 7B Instruct.

Loads the model once, then runs MCTS simulations on each benchmark
sequentially. Supports resume from snapshots for long runs.

Usage:
    # Run all benchmarks to 5000 sims
    python run_qwen25_7b_benchmark_sweep.py --num_simulations 5000
    
    # Resume all incomplete runs to 5000 sims
    python run_qwen25_7b_benchmark_sweep.py --num_simulations 5000 --resume_incomplete
    
    # Run specific benchmarks
    python run_qwen25_7b_benchmark_sweep.py --benchmarks boolq mmlu_all
    
    # Resume a single snapshot
    python run_qwen25_7b_benchmark_sweep.py --resume_snapshot predictions/qwen25_7b_v2_sdpa/benchmark_mcts_boolq_20260307-192947_snapshot.json
    
    CUDA_VISIBLE_DEVICES=0 nohup python run_qwen25_7b_benchmark_sweep.py --num_simulations 5000 --resume_incomplete > nohup_qwen25_7b_5000.log 2>&1 &
"""
import sys as _sys
...
OUTPUT_DIR = "predictions/qwen25_7b_v2_sdpa"
...
def run_benchmark(model, dataset, num_simulations, notify_signal, resume_snapshot=None):
    ...
    if summary["best"]:
        b = summary["best"]
        best_acc = b.get("accuracy_gen") or b.get("accuracy", 0)
        print(
            f"  Best: acc={best_acc:.4f} (delta +{b['delta']:.4f}) "
...
def main():
    p = argparse.ArgumentParser(
        description="Benchmark MCTS sweep for Qwen 2.5 7B",
...
Examples:
  ...
  python run_qwen25_7b_benchmark_sweep.py --exclude_benchmarks arc_easy arc_challenge
"""
    )
    p.add_argument(
        "--num_simulations",
        type=int,
        default=500,
        help="Target number of simulations (default: 500)",
    )
    p.add_argument(
        "--resume_snapshot",
        type=str,
    ...
    p.add_argument(
        ...
        nargs="+",
        default=None,
        help="Run only these benchmarks (e.g. --benchmarks boolq mmlu_all)",
    )
    p.add_argument(
        "--exclude_benchmarks",
        type=str,
    ...
    exclude = args.exclude_benchmarks if args.exclude_benchmarks is not None else []
    ...
    # Cycle through benchmarks in increments of 250 sims instead of completing one at a time
    INCREMENT = 250
        ...
        for idx, (i, dataset, resume_snap) in enumerate(benchmarks_to_run):
            ...
            try:
                ...
                if notify_signal:
                    ...
                    send_signal(
                        f"Qwen2.5-7B {dataset} → {target_sims} sims\n"
                ...
                if notify_signal:
                    send_signal(f"Qwen2.5-7B sweep FAILED on {dataset}: {e}")

    total_elapsed = time.time() - sweep_t0
    print("\n" + "#" * 70, flush=True)
    print("SWEEP COMPLETE -- Qwen 2.5 7B Instruct", flush=True)
    print("#" * 70, flush=True)
